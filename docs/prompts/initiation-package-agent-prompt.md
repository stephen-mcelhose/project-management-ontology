...
Build template packs for the four remaining Initiation Package issues. **Parallelize research (Step 1) across all documents** — fetching PRINCE2 URLs and reading `document.ttl` has no shared state. **Do ontology gap-checking (Step 2) as a single consolidated pass** across every document in this batch before writing any template — documents in the same package frequently need the same or overlapping ontology properties (e.g. shared fields on `pm:Project`), and editing `ontology/modules/*.ttl` concurrently from multiple agents/sessions risks conflicting or duplicate properties. Once the ontology is settled and validated, templates and shapes (Steps 3–4) can again be written in parallel per document:
...
```
templates/initiation/{kebab-name}/entry.yaml
templates/initiation/{kebab-name}/instructions.yaml
templates/initiation/{kebab-name}/template.md
shapes/initiation/{kebab-name}.shacl.ttl
```
...
Template pack built at `templates/initiation/{name}/`.
SHACL shape at `shapes/initiation/{name}.shacl.ttl`.
